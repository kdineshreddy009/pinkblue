...
function newRequest() {
    ...
    $.ajax({
        ...
        success: function(data) {
            ...
            setCookie("pinkBlueUser", data.user, 1);

            if (data.user === "Store Manager") {
                window.location = "/storemanager_view/" + uname;
            } else if (data.user === "Store Assistant") {
...
function fetchInventory() {
    ...
    $.ajax({
        ...
        success: function(data) {
            ...
            let msg = document.getElementById("fetchInvDiv");
            if (data.error) {
        ...
        error: function(data) {
            let msg = document.getElementById("fetchInvDiv");
            msg.innerHTML = "Failed to Fetch products";
            console.log("errored", data.toString());
        }
    });
}
...
//On same webpage : use Ajax. /.  To switch web pages: use window location